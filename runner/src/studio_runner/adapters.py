...
def _run_pytorch_backend(run_id: str, prompt: str, parameters: dict[str, Any]) -> dict[str, Any]:
    mode = settings.sglang_pytorch_adapter_mode.strip().lower()
    if mode not in {"auto", "bench", "mock"}:
        raise AdapterExecutionError(
...
def run_backend_inference(
    ...
    if backend == "sglang-pytorch":
        return _run_pytorch_backend(run_id=run_id, prompt=prompt, parameters=parameters)
